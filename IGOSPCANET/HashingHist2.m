function [f BlkIdx] = HashingHist2(PCANet,ImgIdx,OutImg)
...
NumImg = max(ImgIdx);
f = cell(NumImg,1);
map_weights = 4.^((PCANet.NumFilters(end)-1):-1:0); % weights for binary to decimal conversion
for Idx = 1:NumImg
    ...
    for i = 1:NumImginO 
        ...
        Bhist{i} = sparse(histc(im2col_general(T,PCANet.HistBlockSize,...
            round((1-PCANet.BlkOverLapRatio)*PCANet.HistBlockSize)),(0:4^PCANet.NumFilters(end)-1)')); 
...
loc_pos_x = X_real>0;
loc_pos_y = X_imag>0;
X(loc_pos_x) = X(loc_pos_x)+2;
X(loc_pos_y) = X(loc_pos_y)+1;
